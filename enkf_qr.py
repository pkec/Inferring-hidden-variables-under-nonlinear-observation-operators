...
def EnKF_qr(Af, d, Cdd, h, rng):
    """Quadratic-regression EnKF. Identical to EnKF up to the analysis: the linear factored gain is
    replaced by a least-squares regression of the state on quadratic obs features [1, y, y^2].
...
def run_enkf_qr(obs, truth, obs_idx, h, seed=cfg.seed, N=cfg.ensembleN,
    ...
    """Run the quadratic-regression EnKF over an assimilation window. Returns dict of per-step
    arrays. Same loop as run_enkf; only the analysis step (EnKF_qr) differs.
